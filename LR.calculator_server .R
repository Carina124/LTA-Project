...
LR_calculator <- function(file.name, num_contribs, num_sims, is.a.truecontrib){
  ...
  ##########################
    ...
    while (j < num_sims){ 
      ...
      # this need to change when the populations are mixed 02/18/20
...
true.array_3d = array(rep(1,num_sims*8*length(pops.for.sims)), dim =c(num_sims,8,length(pops.for.sims)))
...
write.csv(true.array_3d,"truecontrib.array.csv")
saveRDS(true.array_3d, file = "truecontrib.array.csv")
...
#setting up the empty 3d array, rows are pop, columns are # of contrib, iteate-1 is the sheets of the cube
non.contrib.fpr.array = array(rep(1,num_sims*8*length(pops.for.sims)), dim =c(num_sims,8,length(pops.for.sims)))
...
for (y in 1:length(pops.for.sims)) {
  print(y)
  for (x in 1:length(contributer)) {
    result = LR_calculator(pops.for.sims[y], x, num_sims, 1)
    non.contrib.fpr.array[,x,y] = result
    
  }
}
write.csv(non.contrib.fpr.array,"non.contrib.fpr.array.csv")
saveRDS(non.contrib.fpr.array, file = "non.contrib.fpr.array.csv")
...
fprcount_mat = matrix(1:(length(pops.for.sims)*8),nrow = length(pops.for.sims),ncol = 8)
