...
###############################04.23.20 (NC) Total contrib For X POPULATION ##############################################################
library(forensim)
library(readr)
...
#### START OF FUNCTION ####
###########################
LR_calculator_cross <- function(file.name, ref.pop,num_contrib, num_sims, is.a.truecontrib){
  library(forensim)
  library(readr)
  
  #reading in pop names
  #pops.for.sims <- readRDS("pop.for.sims.csv")
  
  # We have to use this particular seed for reproduceability 
  set.seed (123560) 
  
  #reads in simuation population allele frequency table
  afs_csv <- read.csv(file.name)
  afs_csv

  ########## adding in a loop that replaces zeros with forensim for sim pop NC  #########
  
   afs_nvalues_current_pop <-Nvalues_for.sims[file.name,] #the nvalues for the simulation population
  
  
  for (x in 1:length(loci.names)) { # this loop looks at only the 13 str loci we will use to simulate people 
    
    locus= loci.names[x] 
    print(locus)
    
    
    for(i in 2:length(colnames(afs_csv))){ #connecting loci.names.in order to the colnames in the ref.pop.csv file
      
      
      if(colnames(afs_csv[i]) == locus){ #we will change n values ONLY for STRS in Loci.names in order 
        
        
        for (j in 1:length(afs_csv[[locus]])){
          
          allele_freq <-afs_csv[[locus]][[j]]
          ...
          if(allele_freq == 0.0){
            ...
            afs_csv[[locus]][j] <-afs_nvalues_current_pop[[locus]]
  ...
  ########## adding in a loop that replaces zeros with forensim for ref. pop NC  ###################
  
  ref.pop.csv <- read.csv(ref.pop)   #reads in the reference population allele frequency table
  
  currentpop_zero_sub <-Nvalues_for.sims[ref.pop,] # N_values for the reference population 
  ...
  for (x in 1:length(loci.names)) { # this loop looks at only the 13 str loci we will use to simulate people 
    ...
    for(i in 1:length(colnames(ref.pop.csv))){ #connecting loci.names.in order to the colnames in the ref.pop.csv file
      
      
      if(colnames(ref.pop.csv[i]) == locus){ #we will change n values ONLY for STRS in Loci.names in order 
        
        
        for (j in 1:length(ref.pop.csv[[locus]])){
          
          allele_freq <-ref.pop.csv[[locus]][[j]]
          ...
          if(allele_freq == 0.0){
            ...
            ref.pop.csv[[locus]][j] <-currentpop_zero_sub[[locus]]
  ...
  #4/23/20 NC -- aim 3 
  ref.pop.afs <- tabfreq(tab =  ref.pop.csv, 
  ...
  loci.names.inorder = pop.afs@which.loc
    ...
    while (j < num_sims){
      ...
      while (k < 14){  
        ...
        ######################################  
         ...
         if(is.na(single_LR$LR) || is.infinite(single_LR$LR) || is.nan(single_LR$LR) || single_LR$LR<0){
           print(paste("In While Loop"))
           loop_count = 1
...
LR_calculator <- function(file.name, num_contribs, num_sims, is.a.truecontrib)
  ...
  result_4contrib <- LR_calculator(file.name,4,10,0)
  result_5contrib <- LR_calculator(file.name,5,100,1)
  result_6contrib <- LR_calculator(file.name,6,100,1)
  result_7contrib <- LR_calculator(file.name,7,100,1)
  result_8contrib <- LR_calculator(file.name,8,100,1)
  result_9contrib <- LR_calculator(file.name,9,100,1)
  result_10contrib <- LR_calculator(file.name,10,100,1)
  
  
  
  ############################TESTING CROSS POP ADJUSTMENTS###################################################

  file.name = "USAf.1_new.csv" #test 273 populations
  ref.pop = "Albania_new.csv"
  # These files have the zeros replaced
  ref.pop = population_afs
  num_contrib = 4
  num_sims =2 
  is.a.truecontrib = 0
 test_cross <- LR_calculator_cross(file.name, ref.pop,num_contrib, num_sims, is.a.truecontrib)
  
 cross_3contrib <- LR_calculator_cross(file.name,ref.pop,3,10,0)
 cross_4contrib <- LR_calculator_cross(file.name,ref.pop,4,10,0)
 cross_5contrib <- LR_calculator_cross(file.name,ref.pop,5,10,0)
 cross_6contrib <- LR_calculator_cross(file.name,ref.pop,6,10,0)
 cross_7contrib <- LR_calculator_cross(file.name,ref.pop,7,10,0)
 cross_8contrib <- LR_calculator_cross(file.name,ref.pop,8,10,0)
 cross_9contrib <- LR_calculator_cross(file.name,ref.pop,9,10,0)
 cross_10contrib <- LR_calculator_cross(file.name,ref.pop,10,10,0)
 
 #LR_calculator_cross <- function(file.name, ref.pop,num_contrib, num_sims, is.a.truecontrib)
 
 result <-LR_calculator_cross(pops.for.sims[1], pops.for.sims[2],4, num_sims + 1, 0)
 
 
count = 0
for (i in 1:length(cross_4contrib)){
  if (cross_4contrib[i] > -1) {
    count = count + 1
  ...
  ## NC 7/21/20 nvalues 
  Nvalues_for.sims <- readRDS("Nvalues_df")
  ...
  function(file.name, ref.pop,num_contrib, num_sims, is.a.truecontrib)
  
 
  pops.for.sims = pops.for.sims[1:2]
  ...
  for (y in 1:length(pops.for.sims)) {
    print(y) #simulation population
    print(pops.for.sims[y])
    for(i in 1:length(pops.for.sims)){
      ref.pop.afs <- pops.for.sims[i] #reference population
      print(ref.pop.afs)
    
    for (x in 1:length(contributer)) {
      print(x)
      #LR_calculator_cross <- function(file.name, ref.pop,num_contrib, num_sims, is.a.truecontrib)
      result <-LR_calculator_cross (pops.for.sims[y], ref.pop.afs,x, num_sims + 1, 0)
      print(result)
  ...
  #pops.for.sims = pops.for.sims[1:50]
  ...
  ##################
  # Comparing the first 50 populations with code that has STRs in order 
  
  #saving  my array with 100 iterations 
  non.contrib.fpr.array_273pop_100it <- non.contrib.fpr.array
  
  non.contrib.ave.fpr_mat.50pop <-non.contrib.ave.fpr_mat[1:50,]
