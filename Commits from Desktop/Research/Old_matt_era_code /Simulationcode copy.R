...
library(forensim)
total_contrib_function <- function(file_name, population_size, num_contribs, iterations, non_or_true_contrib) 
{
  afri.gene <- read.csv(file_name)    
  afripop <- tabfreq(tab=afri.gene, pop.names=as.factor('Afri'))
  afrigeno <- simugeno(tab=afripop, n=population_size, which.loc = c("D8S1179", "TH01" ,"FGA", "CSF1PO",
  ...
  if (non_or_true_contrib == 0){
    ...
    while (j < iterations){
      ...
      if (num_contribs == 1){
        ...
        while (k < 14){  
          ...
          single_LR <- LR(Repliste=c(mix.func@mix.all[[k]]),
                          ...
                          Vp=0,Vd=c(as.numeric(strsplit(mix.func$mix.prof[1,k], "/")[[1]])), ## Vp = Non contrib under Hp - 0 ##
                          xd=1,xp=0,theta=0,prDHet=c(0.2,0.2),
                          prDHom=c(0.04,0.04),prC=0,freq=freq_vector[[k,1]])
      ...
      LR_vector_true[j] <- (abs(LR_vector[1])*abs(LR_vector[2])*abs(LR_vector[3])*
                              abs(LR_vector[4])*abs(LR_vector[5])*abs(LR_vector[6])*
                              abs(LR_vector[7])*abs(LR_vector[8])*abs(LR_vector[9])*
                              abs(LR_vector[10])*abs(LR_vector[11])*abs(LR_vector[12])*
                              abs(LR_vector[13]))
    ...
    while (j < iterations){ #loop over simulations
      ...
      print(total_LR_vector)
      j = j + 1
    }
  }
  return(total_LR_vector)
}
total_contrib_function('Afri.gene.csv', 100, 9, 101, 0)
