...
# i think i need to evalaute where the vectors should begin. SingleLR vec I think should be created once within the look 
#or else I think it is going to be a collection of numbers not realated to the specific simulation 
# also the brackets are still off and I think this is because I don't 100% have it figured out where everything starts/ ends 
...
#you can access indidual STR freq data like this: pop.afs$tab$USAf.1_new.csv$VWA
  ...
  pop.afs <- tabfreq(tab=afs_csv, pop.names=as.factor('population'))
 ...
 sim.genotypes <- simugeno(tab=pop.afs, n=pop.size, which.loc = c("CSF1PO","D3S1358","D5S818","D7S820","D8S1179",
  ...
  noncontrib <- simugeno(tab=pop.afs, n=pop.size, which.loc = c("CSF1PO","D3S1358","D5S818","D7S820","D8S1179",
                                                          "D13S317","D16S539","D18S51","D21S11",
                                                          "FGA","TH01","TPOX","VWA"))
  ...
  noncon.sus <- simumix(noncontrib, ncontri = 3)
  ...
  d2s.2 <- pop.afs$tab$population$D5S818         #D2S1338 *changed out for a diff STR
  ...
  d19.2 <- pop.afs$tab$population$D13S317       #D19S433 *
  
  pop.afs.matrix <- matrix(list(), nrow=13, ncol=1)  ## Creating Matrix ## Numeric, 45? Not sure this is working 
  ...
  if (non.or.truecontrib == 0){    #is a true contributor mix   #loop 1: 
    ...
    while (j < num_sims){
     ...
     sim.mix <- simumix(sim.genotypes,ncontri = num_contribs)
      ...
      if (num_contribs == 1){                                     #loop 2:
        ...
        while (k < 14){
          ...
          single_LR <- LR(Repliste=c(sim.mix$mix.all[[k]]),                         
                          ...
                          Td=0,Vp=0,Vd=c(as.numeric(strsplit(sim.mix$mix.prof[1,k], "/")[[1]])),
                          xd=1,xp=0,theta=0,prDHet=c(0.2,0.2),
                          prDHom=c(0.04,0.04),prC=0,freq= pop.afs.matrix[[k,1]])
          print(single_LR$LR)
      ...
      } else {  #if more than one contributor it goes here 
        k = 1 
        ...
        while (k < 14){  
          ...
          for(i in 1:num_contribs){
            ### QUESTION: why do we need to use strsplit ? Rename these alleles so they make sense 
             prosecutor.all.atk= c(  prosecutor.all.atk, as.numeric(strsplit(sim.mix$mix.prof[i,k], "/")[[1]]))
        
          
            } defense.all.atk = c()
          for (i in 1:(num_contribs - 1)){
            defense.all.atk = c(defense.all.atk, as.numeric(strsplit(sim.mix$mix.prof[i,k], "/")[[1]]))   
          ...
          single_LR <<- LR(Repliste=c(sim.mix@mix.all[[k]]),
                          ...
                          Td=c(defense.all.atk), ## Vd = Non contrib under Hd - Suspect ##
                          
                          #tells Dorothy to return the last two digits of vector (dont think we need the strsplit commands)
                          Vp=0,Vd=c(as.numeric(tail(prosecutor.all.atk,2), "/")[[1]]), ## Vp = Non contrib under Hp - 0 ##
                          xd=1,xp=0,theta=0,prDHet=c(0.2,0.2),
                          prDHom=c(0.04,0.04),prC=0,freq= pop.afs.matrix[[k,1]])
          print(single_LR$LR)
          print(single_LR)
  ...
  } else if (non.or.truecontrib == 1) {     ### if we are interested in non contributors #####
    j = 1
    #set.seed(657)
    while (j < simulations){ #loop over simulations
     sim.mix <- simumix(popgeno,ncontri = num_contribs)
      ...
      if (num_contribs == 1){
        ...
        while (k < 14){  
          ...
          prosecutor.all.atk = c()
          for(i in 1:num_contribs){
            prosecutor.all.atk = c(prosecutor.all.atk, as.numeric(strsplit(mix.func$mix.prof[i,k], "/")[[1]]))
            defense.all.atk = c()
            for (i in 1:c){
              defense.all.atk = c( defense.all.atk, as.numeric(strsplit(mix.func$mix.prof[i,k], "/")[[1]]))
  ...
  return(log10_LR) #singleLRs_vec_product[j] - this is what I want reterned 
}



total_contrib_function <- function(file_name,pop_size,num_contribs,simulations,non_or_true_contrib) 
LRs = total_contrib_function("USAf.1_new.csv", 1000, 3, 1000, 0)
...
# z are the slices of 1000 simulations [z,,]
