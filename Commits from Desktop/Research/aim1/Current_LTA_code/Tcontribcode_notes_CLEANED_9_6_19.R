     ...
     if (num_contribs == 1){                                    
       ...
       while (k < 14){
          ...
          single_LR <- LR( Repliste = c(sim.mix$mix.all[[k]]),
                           ...
                           prDHom = c(0.04,0.04),
                           prC = 0,
                           freq = pop.afs@tab[[1]][[k]]#pop.afs@tab[[POP INDEX]][[LOCUS INDEX]]#pop.afs.matrix[[k,1]]
                          )
...
######################################       
  ...
  ##########################
    ...
    while (j < num_sims){ 
      ...
      #the noncontributor is simulated here 
  ...
  #end of else if (is.a.truecontrib == 1)  
  }
...
LRs <- LR_calculator("Apache.csv", number.contributors = 4, iterations = 10, "true")
...
###populations that will be used for simulations 
pops.for.sims

##ishi populations
pops.for.sims <- c("USAf.1_new.csv",
                  "Ojbwe.243_new.csv",
                  "Inuit.1_new.csv",
                  "Nat.Am.1_new.csv",
                  "Russia.1_new.csv",
...
true.array_3d = array(rep(1,1000*8*length(pops.for.sims)), dim =c(1000,8,length(pops.for.sims)))
...
for (y in 1:length(pops.for.sims)) {
  ...
  for (x in 1:length(contributer)) {
    result <-LR_calculator(pops.for.sims[y], x, 1001, 0)
...
for (y in 1:length(pops.for.sims)) {
  ...
  for (x in 1:length(contributer)) {
    LR_calculator(pops.for.sims[y], x, 1001, 1)
