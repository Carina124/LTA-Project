...
#contains all the files we want to plot 
freq_vec_long <- c("Afri_paper.csv","Apache.csv","Bahamian.csv","Caucasian.csv","Chamorro.csv","Filipino.csv",
              "Jamaican.csv","Navajo.csv","seh.csv","swh.csv","Trini.csv")

freq_vec <- c("Afri.gene.csv","asian_nist.csv","Apache.csv")

#starts the graphics device driver for PDF graphics 
pdf("many_loci_D18S51", width=20, height=20)
...
for( j in 1:length(freq_vec)){
  ...
  for (i in 2:dim(file_mat)[2]){
    ...
    if(name == current_loci){
      ...
      alleles <- file_mat[,1]
      name =  colnames(file_mat)[i]
      plot(x= alleles[isthere], y = freq[isthere], type = "h",lwd = 8,col="Blue", xlab = "Alleles",
           ylab = "frequency",main = paste(name, freq_vec[j]), xlim = c(0,40), ylim=c(0,.3))
    }
  }
}
}
