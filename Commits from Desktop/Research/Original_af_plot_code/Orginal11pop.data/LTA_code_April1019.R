...
total_contrib_function <- function(file_name,pop_size,num_contribs,iterations,non_or_true_contrib) #PASS IN TWO POPULATION FILES: SIMPOPFILE AND EVALPOPFILE
{
  set.seed(657)
  allelefreq_tbl <- read.csv(file_name)    
  pop_freq <- tabfreq(tab=allelefreq_tbl, pop.names=as.factor('population'))
  #popgeno <- simugeno(tab=pop_freq, n=pop_size, which.loc = c("D8S1179", "TH01" ,"FGA", "CSF1PO",
  ...
  fga.2 <- pop_freq$tab$population$FGA           ## Inititialize variables for vector ##
  ...
  d2s.2 <- pop_freq$tab$population$D2S1338
  d19.2 <- pop_freq$tab$population$D19S433
  ...
  if (non_or_true_contrib == 0){
    ...
    j = 1         
    while (j < iterations){
      #SIMULATE NCONTRIB+1 INDIVIDUALS INTO POPGENO    SIM USING SIMPOP
      
      mix.func <- simumix(popgeno,ncontri = num_contribs)
      #CHECK THAT EACH INDIV IS IN MIX ONCE
      ...
      if (num_contribs == 1){
        ...
        while (k < 14){
          single_LR <- LR(Repliste=c(mix.func$mix.all[[k]]),                         
                          ...
                          prDHom=c(0.04,0.04),prC=0,freq=freq_vector[[k,1]])  #USING EVALPOP
          #print(single_LR$LR)
      ...
      total_LR_plot_vectornav6 <<- total_LR_vector #LOSE IT
      g = 1
      while (g < 3){
        allelefreq_tbl <- read.csv(file_name)    
        pop_freq <- tabfreq(tab=allelefreq_tbl, pop.names=as.factor('population'))
...
LRsfili = total_contrib_function('Filipino.csv', 2, 5, 1000, 1)
LRstrin = total_contrib_function('Trinidadian', 10000, 5, 1000, 1)
LRsjam = total_contrib_function('Jamaican', 10000, 5, 1000, 1)
...
array_3d = array(rep(1,10000*8*11), dim =c(10000,8,11))
...
for (y in 1:length(pop_vec)) {
  ...
  for (x in 1:length(contributer)) {
       total_contrib_function(pop_vec[y], 100000, x, 10001, 0)
...
#setting up the empty 3d array, rows are pop, columns are # of contrib, iteate-1 is the sheets of the cube
array_3dnon = array(rep(1,10000*8*11), dim =c(10000,8,11))
#view empty array
...
write.csv(FPR7_matrix,"FPRmatrix1000.csv")

FPR7_matrix#This is the vector used to make the different colored lines for all of the populations 
