...
freq_vec <- c("afri_NIST.csv","Apache.csv","Bahamian.csv","Chamorro.csv",
              "Navajo.csv")
...
#contains all the files we want to plot 
freq_vec <- c("afri_paper.csv","Apache.csv","Bahamian.csv","Navajo.csv")

freq_vec2 <- c("African American","Apache","Bahamian","Navajo")

#starts the graphics device driver for PDF graphics 
pdf("11_loci_D3S1358", width=20, height=20)
  ...
  for( j in 1:length(freq_vec)){
    ...
    for (i in 2:dim(file_mat)[2]){
      ...
      if(name == current_loci){
        ...
        alleles <- file_mat[,1]
        name =  colnames(file_mat)[i]
        plot(x= alleles[isthere], y = freq[isthere], type = "h",lwd = 4,col="black", xlab = "Alleles", ylab = "Frequency",
             main = paste(freq_vec2[j]), xlim = c(0,55), ylim=c(0,1))
      }
    }
  }
}
...
file_name <- read_csv("Afri_paper.csv")
#file_name <- read_csv("afri.gene.csv")
file_mat <- as.matrix(file_name)
...
#################################### Matrix building (Friday w/ Maria) 2/22/19 ################################################################
...
exp_heteroZ
length(exp_heteroZ)
# It also fills the vector myvector to be used in plotting STRs 
for (i in 2:dim(file_mat)[2]){
  freq <- file_mat[,i]
  isthere = !is.na(freq)
  #print(isthere)
  ...
  name =  colnames(file_mat)[i]
  print(i)
  plot(x= alleles[isthere], y = freq[isthere], type = "h",lwd = 12,col="red", xlab = "Alleles",
       ylab = "frequency",main = paste(name, file_name), xlim = 55, ylim=1)
...
####Chomorro####
{library(readr)
  Chamorro <- read_csv("Chamorro.csv")
  
  afri_mat <- as.matrix(afri_gene)
  alleles <- afri_mat[,1]
  plot.new()
  par(mfrow=c(4,4))
  ...
  freq <- afri_mat[,3]
  isthere = !is.na(freq)
  plot(x= alleles[isthere], y = freq[isthere], type = "h",lwd = 12,col="red", xlab = "frequency",
       ylab = "Alleles",main = " Allele vs. Chamorro Frequency")
}
...
plot.new()
plot(x= alleles[isthere], y = freq[isthere], type = "h",lwd = 12,col="red")
...
alleles <- afri_mat[,1]
col =2
for(col in ncol(afri_mat)){
  print(afri_mat[col+1])
}
...
plot(alleles,freq)
isthere = !is.na(freq)
plot(x=alleles[isthere], y=freq[isthere])  
############################################################
library(forensim)
library(readr)
Chamorro <- read_csv("Chamorro.csv")
...
afri.gene <- Chamorro 
afripop <- tabfreq(tab=as.matrix(afri.gene), pop.names=as.factor('Afri'))
afrigeno <- simugeno(tab=afripop, n=100, which.loc = c("TPOX","CSF1PO","vWA","TH01","D3S1358",
                                                       "D7S820","D16S539","D18S51","D21S11","D2S1338",
                                                       "D19S433","FGA","D8S1179"))
...
i=1
while(i<14){
  allele_plots <- afri.gene$tab.freq$afri.gene[[i]]
  barplot(allele_plots,las=1)
  i=i+1
}
#############################################################
?is.na
...
###Practice on on specfic file to figure out indivdual components of the for loop 
library(readr)
Caucasian <- read_csv("Caucasian.csv")
View(Caucasian)
...
freq <- afri_mat[,2]
afri_mat[,2]
x_vec <- as.vector(xaxis)
y_vec <- 
  
  plot.new()
hist(alleles, breaks=seq(from=-1,to=50,by=1), col=rgb(1,0,0,0.5), prob = TRUE, ylim=c(0,.1),
     main="FGA", xlab="Allele", ylab = "Frequency")
?hist
name <- colnames(afri_gene[2])