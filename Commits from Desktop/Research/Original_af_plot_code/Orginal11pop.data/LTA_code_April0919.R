...
LRs = total_contrib_function('Trinidadian.csv', 100, 4, 101, 1)
...
for (y in 1:length(pop_vec)) {
  ...
  for (x in 1:length(contributer)) {
       total_contrib_function(pop_vec[y], 10000, x, 1001, 0)
...
#setting up the empty 3d array, rows are pop, columns are # of contrib, iteate-1 is the sheets of the cube
array_3dnon = array(rep(1,1000*8*11), dim =c(1000,8,11))
#view empty array
...
for (y in 1:length(pop_vec)) {
  ...
  for (x in 1:length(contributer)) {
    total_contrib_function(pop_vec[y], 10000, x, 1001, 1)
